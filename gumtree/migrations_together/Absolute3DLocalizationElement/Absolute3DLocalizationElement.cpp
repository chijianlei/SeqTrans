...
namespace sumet_state_estimator
{
      ...
      bool verbose)
  {
    ...
    tf::Vector3 x_vec = T_cur.getOrigin();
    ...
    tf::Vector3 Xvec(X_out(0), X_out(1), X_out(2));
    tf::Quaternion Q;
    Q.setRPY(X_out(5), X_out(4), X_out(3));
    T_out = tf::Transform(Q, Xvec);
  }
      ...
      bool verbose)
  {
    ...
    tf::Vector3 X(X_cur(0), X_cur(1), X_cur(2));
    tf::Quaternion Q;
  ...
  //////////////////////////////////////////////////////////////////////////////
  void Absolute3DLocalizationElement::load_pos_and_cov_data(
      const tf::Transform& T_in, const LaGenMatDouble& Cov_in)
  {
    tf::Vector3 Xtemp = T_in.getOrigin();
      ...
      const geometry_msgs::PoseWithCovarianceStamped& msg)
  {
    ...
    const geometry_msgs::Point& x = msg.pose.pose.position;

    X_(0) = x.x;
    X_(1) = x.y;
    X_(2) = x.z;

    const geometry_msgs::Quaternion& Qm = msg.pose.pose.orientation;
    tf::Transform transform(tf::Quaternion(Qm.x, Qm.y, Qm.z, Qm.w));
  ...
  void Absolute3DLocalizationElement::set_position_data(
    const tf::Transform& T_in)
  {
    T_ = T_in;
    const int32_t num_states = 6;
    LaVectorDouble X_cur = LaVectorDouble::zeros(num_states, 1);
    tf::Vector3 x_vec = T_in.getOrigin();
