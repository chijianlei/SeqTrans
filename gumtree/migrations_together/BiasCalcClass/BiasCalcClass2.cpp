...
namespace sumet_state_estimator
{
  ...
  {
    start_acquisition_time_ = swri::TIME_MIN;//ros::Time::now();
    is_active_ = false;
    min_bias_time_ = 10.0;
    max_bias_time_ = 60.0;
  ...
  void BiasCalcClass::initialize(int NumSamplesToAverage, rclcpp::Time now)
  {
    start_acquisition_time_ = now;
  ...
  rclcpp::Duration BiasCalcClass::get_bias_age(rclcpp::Time curTime)
  {
    //rclcpp::Time curTime = ros::Time::now();
    return (curTime - bias_computation_time_);
  }

  bool BiasCalcClass::get_is_bias_current(rclcpp::Time curTime)
  {
    return (swri::toSec(get_bias_age(curTime)) < bias_validity_duration_);
  }
  ...
  bool BiasCalcClass::load_new_data(double data, rclcpp::Time curTime, bool force)
  {
    //rclcpp::Time curTime = ros::Time::now();
    // Time elapsed from the last sample added
    double cur_duration = swri::toSec(curTime - last_data_time_);
