...
namespace sumet_state_estimator
{
  ...
  void DerivativeEstimator::add_new_data(const rclcpp::Time& time1,
                                         double curData)
  {
    sumet_state_estimator::StampedDoubleData data1(time1);
  ...
  double DerivativeEstimator::get_derivative(int N)
  {
    ...
    double dt = swri::toSec(rclcpp::Time(dat2.header.stamp) 
                          - rclcpp::Time(dat1.header.stamp));
    if (dt <= 1.0e-15)
    {
