...
namespace sumet_state_estimator
{
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  bool LocalizationQueue6DOF::run_filter_step(const ros::Time &filter_until)
  {
    if (size() == 0)
    {
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  bool LocalizationQueue6DOF::process_queue(ros::Time T1,
  ...
  void LocalizationQueue6DOF::regularize_list(
      const ros::Time& regularize_until_time)
  {
    ...
    if (size() > 0)
    {
      ...
      do
      {
        ...
        for (int i = 0; i < size(); ++i)
        {
          ...
          ros::Time last_time = ReturnElement(i)->getTimestamp();
  ...
  bool LocalizationQueue6DOF::find_idx_for_timestamp(
      const ros::Time& time_to_find, int& idx, int mode)
  {
    idx = 0;
      ...
      geometry_msgs::PoseWithCovarianceStamped& pose) const
  {
    ...
    geometry_msgs::Pose& p = pose.pose.pose;
    p.position.x = X_(0);
    p.position.y = X_(1);
    p.position.z = X_(2);
  ...
  tf::Transform getTransform(const LaVectorDouble& X)
  {
    tf::Transform T;
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  int LocalizationQueue6DOF::get_closest_elem(const ros::Time &T)
  {
    int min_idx = -1;
    double MinDiff = 1e20;
    for (int i = 0; i < size(); ++i)
    {
      double diff = (ReturnElement(i)->getTimestamp() - T).toSec();
      if (diff < 0.0)
      {
        if (abs(diff) < MinDiff)
        {
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  double LocalizationQueue6DOF::getDT(int idx, const ros::Time& Tmax,
      ros::Time& T_actual)
  {
    double dt = 0.0;
    if (size() > idx + 1)
    {
      ros::Time nextTime = ReturnElement(idx + 1)->getTimestamp();
      ros::Time curTime = ReturnElement(idx)->getTimestamp();
      if (nextTime < Tmax)
      {
        T_actual = nextTime;
        dt = (nextTime - curTime).toSec();
      }
      else
      {
        T_actual = Tmax;
        dt = (Tmax - curTime).toSec();
      }
    }
    else if (size() > idx)
    {
  ...
  void LocalizationQueue6DOF::PrintTimestampsAndTypes()
  {
    if (size() > 0)
    {
      ...
      for (int i = 0; i < size(); ++i)
      {
        uint64_t time = ReturnElement(i)->getTimestamp().toNSec();
