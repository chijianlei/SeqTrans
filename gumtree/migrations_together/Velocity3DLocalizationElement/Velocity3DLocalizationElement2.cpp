...
namespace sumet_state_estimator
{
  ...
  //////////////////////////////////////////////////////////////////////////////
  Velocity3DLocalizationElement::Velocity3DLocalizationElement(
      const geometry_msgs::msg::TwistWithCovarianceStamped& Vinit)
  {
    pre_init();
  ...
  //
  //////////////////////////////////////////////////////////////////////////////
  void Velocity3DLocalizationElement::convert_cov(const std::array<double,
                                                  36>& cov_in)
  {
      ...
      geometry_msgs::msg::TwistWithCovarianceStamped& twist) const
  {
    ...
    std::array<double, 36>& cov = twist.twist.covariance;
    for (int i = 0; i < 6; i++)
    {
      for (int j = 0; j < 6; j++)
      {
  ...
  void Velocity3DLocalizationElement::run_update_step(
    ...
    bool verbose)
  {
    tf2::Vector3 X_0 = T_cur.getOrigin();
    tf2::Vector3 X_out(0.0,
                    0.0,
                    0.0);

    tf2::Quaternion Q_0 = T_cur.getRotation();
    double roll;
    double pitch;
    double theta;

    tf2::Transform(Q_0).getBasis().getRPY(roll, pitch, theta);

    theta = sumet_util::MathUtil::FixAngle0to2Pi(theta);

    tf2::Vector3 rpy(roll,
                  pitch,
                  theta);

    tf2::Quaternion Q_out = tf2::Quaternion::getIdentity();

    tf2::Vector3 v_in(V_.linear.x,
                   V_.linear.y,
                   V_.linear.z);

    tf2::Vector3 w_in(V_.angular.x,
                   V_.angular.y,
                   V_.angular.z);
    ...
    T_out = tf2::Transform::getIdentity();
    ...
    if (age > 0.1)
    {
      ...
      ROS_ERROR_THROTTLE(
        ...
        this->getTimestamp().nanoseconds()/1000000000.0,
        this->orig_stamp_.nanoseconds()/1000000000.0);
        //ros::Time::now().toSec());
    }
  ...
  void Velocity3DLocalizationElement::set_velocities(
      const geometry_msgs::msg::Twist& Vin)
  {
    V_ = Vin;
  }
  ...
  void Velocity3DLocalizationElement::set_velocity_data(
      const geometry_msgs::msg::TwistWithCovarianceStamped& Vin)
  {
  ...
  {
    return swri::toSec(this->getTimestamp() - this->orig_stamp_);
  }
