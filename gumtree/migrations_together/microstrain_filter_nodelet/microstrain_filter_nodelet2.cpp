...
namespace sumet_state_estimator
{
   ...
   *  Microstrain AHRS
   */
  class MicrostrainFilterNodelet : public swri::Node
  {
    public:
      MicrostrainFilterNodelet():
        last_vel_update_(0, 0, RCL_ROS_TIME),
        last_lat_accel_(0),
        last_long_accel_(0),
        last_linear_vel_(0),
        last_angular_vel_(0),
        last_imu_update_(0, 0, RCL_ROS_TIME),
        stopped_time_(0.0),
        stop_time_(0, 0, RCL_ROS_TIME),
        imu_to_vehicle_trans_(tf2::Transform::getIdentity()),
        init_roll_(0),
        init_pitch_(0),
        init_yaw_(0),
        g_magnitude_(9.8),
        ...
        max_vel_interval_(0),
        max_latency_(0),
        last_imu_msg_(new sensor_msgs::msg::Imu),
        Node("microstrain_filter")
      {
      }
    ...
    private:
      ...
      // Publishers
      rclcpp::Publisher<sensor_msgs::msg::Imu>::SharedPtr Revised_IMU_;

      // Storage for the latest measurements that we'll use
      rclcpp::Time last_vel_update_;
      ...
      BiasCalcClass wyb_;
      BiasCalcClass wzb_;
      geometry_msgs::msg::Vector3 cur_bias_;

      tf2::Transform imu_to_vehicle_trans_;
      double init_roll_;
      double init_pitch_;
      double init_yaw_;
      ...
      double wyb0_;
      double wzb0_;
      geometry_msgs::msg::Vector3 wzb_vec_;
      ...
      {
        // Read parameters:
        auto& pnh = this->nh_;//getPrivateNodeHandle();
        ...
        wxb_.initialize(bias_est_num_samples, nh_->now());
        wyb_.initialize(bias_est_num_samples, nh_->now());
        wzb_.initialize(bias_est_num_samples, nh_->now());
        ...
        last_vel_update_ = rclcpp::Time(0,0, RCL_ROS_TIME);//ros::TIME_MIN;
        last_imu_update_ = rclcpp::Time(0,0, RCL_ROS_TIME);//ros::TIME_MIN;
      }

      void subscribe_to_topics()
      {
        Microstrain_Subscriber_ = swri::Subscriber(this,"imu_in", 5,
            &MicrostrainFilterNodelet::Microstrain_cb, this);
        imu_monitor_.Begin(nh_->now().nanoseconds());

        Velocity_Messages_Subscriber_ = swri::Subscriber(this,"VehicleVelocity6D", 5,
            &MicrostrainFilterNodelet::Velocity_cb, this);
        vel_monitor_.Begin(nh_->now().nanoseconds());
      }
      ...
      {
        Revised_IMU_ = swri::advertise<sensor_msgs::msg::Imu>(this, "imu_out", 100);
        output_monitor_.Begin(nh_->now().nanoseconds());
      }
      ...
      {
        imu_monitor_.EndBegin(msg->header.stamp.nanosec);
        latency_monitor_.Begin(msg->header.stamp.nanosec);
        last_imu_update_ = msg->header.stamp;

        sensor_msgs::msg::Imu::SharedPtr rev_imu(new sensor_msgs::msg::Imu(*msg));
        ...
        if (legacy_mode_)
        {
          ...
          tf2::Quaternion Qtmp(rev_imu->orientation.x,
          ...
          double p;
          double y;
          tf2::Transform(Qtmp).getBasis().getRPY(r, p, y);
          Qtmp.setRPY(r, -p, -y);
        ...
        latency_monitor_.End(nh_->now().nanoseconds());
        output_monitor_.EndBegin(nh_->now().nanoseconds());
      }

      void handle_biases(sensor_msgs::msg::Imu::SharedPtr& msg)
      {
        ...
        if (stopped_time_ > 5.0)
        {
          ...
          wxb_.load_new_data(msg->angular_velocity.x, now, true);
          wyb_.load_new_data(msg->angular_velocity.y, now, true);
          wzb_.load_new_data(msg->angular_velocity.z, now, true);
        }
      ...
      void transform_measurement(sensor_msgs::msg::Imu::SharedPtr& msg,
    ...
    //    NODELET_ERROR("wz after = (%g)", W.getZ());
        ...
        tf2::Matrix3x3 Cw = swri_transform_util::Get3x3Cov(
            msg->angular_velocity_covariance);
        tf2::Matrix3x3 Ca = swri_transform_util::Get3x3Cov(
            msg->linear_acceleration_covariance);
        tf2::Matrix3x3 Cq = swri_transform_util::Get3x3Cov(
            msg->orientation_covariance);

        tf2::Matrix3x3 Cw_rot = aligned_rotation.transposeTimes(Cw)*aligned_rotation;  // NOLINT
        tf2::Matrix3x3 Ca_rot = aligned_rotation.transposeTimes(Ca)*aligned_rotation;  // NOLINT
        tf2::Matrix3x3 Cq_rot = aligned_rotation.transposeTimes(Cq)*aligned_rotation;  // NOLINT
      ...
      void remove_non_gravity_accels(sensor_msgs::msg::Imu::SharedPtr& msg)
      {
        if ((rclcpp::Time(msg->header.stamp) - last_vel_update_).nanoseconds() > 0.15*1000000000.0)
        {
      ...
      void update_covariance(sensor_msgs::msg::Imu::SharedPtr& msg)
      {
        ...
        tf2::Vector3 measured_accel(msg->linear_acceleration.x,
            msg->linear_acceleration.y,
            msg->linear_acceleration.z);
      ...
      void estimate_pitch_and_roll(sensor_msgs::msg::Imu::SharedPtr& msg)
      {
        tf2::Vector3 g1(0.0, 0.0, 1.0);  // gravity unit vector
        tf2::Vector3 measured_accel(
            msg->linear_acceleration.x,
            msg->linear_acceleration.y,
            msg->linear_acceleration.z);

        tf2::Vector3 g2(measured_accel);
        g2.normalize();  // measured accel unit vector

        // Get the rotation axis
        tf2::Vector3 rot_axis = g2.cross(g1);
        ...
        tf2::Transform T(Q_est.inverse());
        tf2::Vector3 test_vec2 = T * g1;
          ...
          const geometry_msgs::msg::TwistWithCovarianceStamped::SharedPtr msg)
      {
        ...
        vel_monitor_.EndBegin(msg->header.stamp.nanosec);
        ...
        {
          stopped_time_ = ((double)(nh_->now() - stop_time_).nanoseconds())/1000000000.0;
        }
      ...
      void setup_diagnostics()
      {
        ...
        diagnostics_timer_ = create_wall_timer(1.0,
            std::bind(&MicrostrainFilterNodelet::diagnostic_update_cb, this));
      }
      ...
      void imu_diagnostic_cb(diagnostic_updater::DiagnosticStatusWrapper &stat)  // NOLINT
      {
        stat.add("Time since last IMU update (s)",
            (nh_->now() - last_imu_update_).nanoseconds()/1000000000.0);
      ...
      {
        double bias_age = wxb_.get_bias_age(nh_->now()).nanoseconds();
        bias_age /= 1000000000.0;
      ...
      void velocity_diagnostic_cb(diagnostic_updater::DiagnosticStatusWrapper &stat)  // NOLINT
      {
        stat.add("Time since last velocity update (s)",
            (nh_->now() - last_vel_update_).nanoseconds()/1000000000.0);
      ...
      void setCov3x3DiagVals(std::array<double, 9>& cov_in,
      ...
      void getCov3x3DiagVals(const std::array<double, 9>& cov_in,
          double& v1, double& v2, double& v3) const
      {
        v1 = cov_in[0];
        v2 = cov_in[4];
...
CLASS_LOADER_REGISTER_CLASS(sumet_state_estimator::MicrostrainFilterNodelet, swri::Node)
