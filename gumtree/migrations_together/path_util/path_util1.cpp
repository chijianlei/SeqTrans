...
#include <tf/transform_datatypes.h> //for tf::Vector3

namespace swri_nav_util
{

void reduceSegments(
  swri_nav_msgs::Path& path)
{
  const swri_nav_msgs::Path path_cpy = path; //copies
  path.segments.clear();
  path.segments.reserve(path_cpy.segments.size());

  for (const swri_nav_msgs::PathSegment& segment_to_append : path_cpy.segments)
  {
    ...
    else
    {
      ...
      //abbreviate
      typedef std::vector<swri_nav_msgs::PathPoint> PointVec;
...
double updateDistance(
    ...
    swri_nav_msgs::PathSegment& segment)
{
    ...
    swri_nav_msgs::Path& path)
{
  ...
  for (swri_nav_msgs::PathSegment& segment : path.segments)
  {
...
void removeCoincidentPoints(
    ...
    const double tol)
{
  swri_nav_msgs::PathSegment new_segment;
...
bool getBackPoint(
    ...
    swri_nav_msgs::PathPoint& back_pt) //output
{
...
void updateYaw(swri_nav_msgs::PathSegment& segment)
{
  double yaw;
  for (size_t ptno=0; ptno+1 < segment.points.size(); ptno++)
  {
    //abbreviate
    swri_nav_msgs::PathPoint& pt = segment.points[ptno];
    swri_nav_msgs::PathPoint& next_pt = segment.points[ptno+1];
...
void updateYaw(swri_nav_msgs::Path& path)
{
...
void separationFromLineSegment(
  const swri_nav_msgs::PathPoint point0,
  const swri_nav_msgs::PathPoint point1,
...
{
  tf::Vector3 x0(point0.x, point0.y, 0.0);
  tf::Vector3 x1(point1.x, point1.y, 0.0);
  tf::Vector3 p(x, y, 0.0);

  tf::Vector3 v = x1 - x0; //vector from point0 to point1
...
inline bool interpolatePathPointPair(
  const swri_nav_msgs::PathPoint p0,
  const swri_nav_msgs::PathPoint p1,
  const double distance,
  swri_nav_msgs::PathPoint &out_point)
{
...
bool findLocalNearestDistanceForward(
    const swri_nav_msgs::PathSegment& segment,
    const double x, const double y,
    const double start_distance,
    ...
    double& nearest_separation)
{
  ...
  for (size_t ptno = 0; ptno+1 < num_points; ptno++)
  {
    ...
    if (this_distance < start_distance)
    {
      ...
      swri_nav_msgs::PathPoint this_pt;
    ...
    double& nearest_separation)
{
  ...
  swri_nav_msgs::PathSegment segment_rev = segment;
...
bool interpolatePathSegment(
    const swri_nav_msgs::PathSegment& segment,
    const double distance,
    swri_nav_msgs::PathPoint& out_point) //output
{
  //special cases
  size_t num_points = segment.points.size();
  if (num_points == 0)
...
{
  swri_nav_msgs::PathSegment out = segment;
  if (segment.points.size() <= 1)
  {
...
void transformPathPoint(
    ...
    swri_nav_msgs::PathPoint& point)
{

  //transform x,y
  tf::Point p0(point.x, point.y, 0.0);
  tf::Point p1 = transform * p0;

  point.x = p1.x();
  point.y = p1.y();

  //transform yaw
  tf::Quaternion q0 = tf::createQuaternionFromYaw(point.yaw);
  tf::Quaternion q1 = transform * q0;

  point.yaw = tf::getYaw(q1);
}
...
void makeArcPathSegment(
    ...
    const double len,
    const double dist_step,
    const swri_nav_msgs::PathPoint& align_pt,
    swri_nav_msgs::PathSegment& segment)
{
  ...
  for (swri_nav_msgs::PathPoint& pt : segment.points) //mutable reference
  {
...
{
  swri_nav_msgs::PathSegment new_segment;
  swri_nav_msgs::PathPoint back_pt;
