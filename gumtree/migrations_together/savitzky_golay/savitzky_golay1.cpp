    ...
    std::vector<double>& fsmooth) //smoothed f
{
  ...
  if (frame_size == 5) { denom = 35; a = {17, 12, -3}; }
    ...
    ROS_ERROR("frame_size input = %d invalid. must be odd and between 5-15", frame_size);
    return false;
  }
  ...
  if (numel < m+1)
  {
    ROS_ERROR("f is too small for frame_size (numel = %d < (frame_size-1)/2 = %d)", numel, m);
    return false;
  }
    ...
    std::vector<double>& fd) //first derivative of f
{
  ...
  {
    ROS_ERROR("frame_size input = %d invalid. must be odd and >= 3", frame_size);
    return false;
  }
