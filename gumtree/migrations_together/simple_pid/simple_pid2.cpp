...
namespace speed_controller
{
  ...
  std::shared_ptr<marti_common_msgs::msg::Float32Stamped> makeFloat32StampedPtr(
      rclcpp::Time stamp,
      double value)
  {
    std::shared_ptr<marti_common_msgs::msg::Float32Stamped> msg = std::make_shared<marti_common_msgs::msg::Float32Stamped>();
    msg->header.stamp = stamp;
    msg->value = value;
    return msg;
  }

  ControlMapConfig::ControlMapConfig(rclcpp::Logger logger):
    min_brake_(0.0),
    max_brake_(1.0),
    brake_stop_(0.6),
    min_throttle_(0.0),
                                ...
                                double min_throttle, double max_throttle, double min_stop_brake)
  {
    ...
    {
      {
        RCLCPP_WARN(logger_,"min_brake (%f) cannot be greater than max_brake (%f); changing min_brake to %f",
    ...
    {
      {
        RCLCPP_WARN(logger_, "min_throttle (%f) cannot be greater than max_throttle (%f); changing min_throttle to %f",
  ...
  {
    {
      RCLCPP_INFO(logger_, "Control Map Configuration: \n"
               "min_brake = %lf\n"
               "max_brake = %lf\n"
               "min_throttle = %lf\n"
               "max_throttle = %lf\n"
  ...
  PidConfig::PidConfig(rclcpp::Logger logger):
    kp_(0.0),
    ki_(0.0),
    kd_(0.0),
    kp_stop_(0.0),
  ...
  void PidConfig::SetGains(double kp, double ki, double kd)
  {
    ...
    {
      {
        RCLCPP_WARN(logger_, "Control gains must be >= 0. Requested: kp = %lf, ki = %lf, kd = %lf",
            kp, ki, kd);
      }
    }
  ...
  void PidConfig::SetGainsStop(double kp_stop, double ki_stop, double kd_stop)
  {
    ...
    {
      {
        RCLCPP_WARN(logger_, "Sum of (kx + kx_stop) must be >= 0. Requested: kp_stop = %lf, ki_stop = %lf, kd_stop = %lf",
                            ...
                            double min_d, double max_d)
  {
    ...
    {
      {
        RCLCPP_WARN(logger_, "min_x values can't be > 0. Requested min_i = %f, min_d = %f", min_i, min_d);
      }
    }
    if (max_i < 0 || max_d < 0)
    {
      {
        RCLCPP_WARN(logger_, "max_x values can't be < 0. Requested max_i = %f, max_d = %f", max_i, max_d);
      }
    }
  ...
  {
    {
      RCLCPP_INFO(logger_, "PID Configuration: \n"
               "kp = %lf\n"
               "ki = %lf\n"
               "kd = %lf\n"
               "kp_stop = %lf\n"
  ...
  SimplePid::SimplePid(rclcpp::Logger logger,rclcpp::Time now):
    last_measured_(0.0),
    last_commanded_(0.0),
    initialized_(false),
    last_err_(0.0),
  ...
  double SimplePid::Update(const rclcpp::Time& time)
  {
    ...
    else
    {
      ...
      double dt = ((time - last_update_time_).nanoseconds())*1.0e-9;
