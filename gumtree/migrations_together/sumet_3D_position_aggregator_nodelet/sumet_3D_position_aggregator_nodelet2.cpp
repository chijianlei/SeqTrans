 ...
 */
    class PoseAggregator : public swri::Node
    {
    private:
        // Subscriptions
         ...
         *                              receiver)
         */
        void Novatel_cb(const gps_common_msgs::msg::GPSFix::SharedPtr GPS)
        {
            if (local_xy_->Initialized())
            {
                ...
                geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr cur_pose(
                ...
                {
                    tf2::Quaternion quat;
                    tf2::fromMsg(cur_pose->pose.pose.orientation, quat);
                    tf2::Transform transform1(quat);

                    // Create a 180.0 rotation.
                    tf2::Vector3 axis(0.0, 0.0, 1.0);
                    tf2::Transform transform2(tf2::Quaternion(axis, M_PI));
                    transform1 = transform2 * transform1;

                    cur_pose->pose.pose.orientation = tf2::toMsg(transform1.getRotation());
                }
            ...
            {
                ROS_ERROR("Position aggregator waiting for local origin");
            }
        }
         ...
         */
        void get_gps_variances(const gps_common_msgs::msg::GPSFix& GPS, double& yaw_var,
         ...
         */
        void GPSFix2VehiclePose(geometry_msgs::msg::PoseWithCovarianceStamped& msg_out,  // NOLINT
                                const gps_common_msgs::msg::GPSFix& gps_fix)
        {
            ...
            geometry_msgs::msg::Point& X = msg_out.pose.pose.position;
            ...
            tf2::Quaternion Q_out = tf2::Quaternion::getIdentity();
            Q_out.setRPY(roll, pitch, yaw);

            tf2::Vector3 X_out(X.x, X.y, X.z);
        ...
        void Imu_msg_handler(const sensor_msgs::msg::Imu& msg)
        {
            last_imu_update_ = nh_->now();
            geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr cur_pose(new geometry_msgs::msg::PoseWithCovarianceStamped());
            ...
            Q = Q * tf2::Quaternion(tf2::Vector3(1.0, 0.0, 0.0), sumet_util::_pi);

            tf2::Transform(Q).getBasis().getRPY(roll, pitch, yaw);
        ...
        void subscribe_to_topics()
        {
            ...
            Novatel_Subscriber_ = swri::Subscriber(this, "gps", 100,
                                                &PoseAggregator::Novatel_cb, this);

            // Subscribes to and IMU message (for pitch and roll information)
            Imu_Subscriber_ = swri::Subscriber(this, "imu", 100,
        ...
        {
            pose_publisher_ = swri::advertise<geometry_msgs::msg::PoseWithCovarianceStamped>(this,
        ...
        void init_node()
        {
            ...
            last_imu_update_ = swri::TIME_MIN;
        ...
        void GpsDiagnostic(diagnostic_updater::DiagnosticStatusWrapper& status)  // NOLINT
        {
            ...
            double time_since_last_gps_update = swri::toSec(cur_time - last_gps_update_);
        ...
        {
            rclcpp::Time cur_time = nh_->now();
            double time_since_last_imu_update = swri::toSec(cur_time - last_imu_update_);
        ...
        void init_diagnostics()
        {
            ...
            diagnostic_timer_ =
                create_timer(1.0,
                                std::bind(&PoseAggregator::RunDiagnostics,
                                this));
        }
         ...
         */
        void setPose(geometry_msgs::msg::Pose& p, const tf2::Vector3& X,
         ...
         */
        void setDiagCov(std::array<double, 36>& cov_out, double v1 =  // NOLINT
         ...
         */
        void getCov3x3DiagVals(const std::array<double, 9>& cov_in, double& v1,
...
CLASS_LOADER_REGISTER_CLASS(sumet_state_estimator::PoseAggregator, swri::Node)
