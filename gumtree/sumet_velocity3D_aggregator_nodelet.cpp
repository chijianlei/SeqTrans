...
#include <marti_sensor_msgs/Gyro.h>
...
class TwistAggregator : public nodelet::Nodelet
{
 public:
  TwistAggregator()
    :
    ...
    cur_latitude_(29.44),
    last_odom_time_(0),
    last_published_twist_(new geometry_msgs::TwistWithCovarianceStamped())
  {
    verbose_ = false;
 ...
 private:
  ...
  geometry_msgs::Vector3Stamped last_microstrain_vel_;
  ...
  static const double ODOM_TIMEOUT = 0.1;  // Seconds
  ...
  ros::Time last_longitudinal_vel_update_;
  ros::Time last_yaw_rate_update_;
  ...
  geometry_msgs::TwistWithCovarianceStampedPtr last_published_twist_;
   ...
   */
  void Novatel_cb(const gps_common::GPSFixConstPtr &msg)
  {
    // Add the yaw rate to the appropriate velocity stream
    // First create a VelocityElem3D
    sumet_state_estimator::VelocityElem3D temp_elem;
   ...
   */
  void gyro_cb(const marti_sensor_msgs::GyroConstPtr& msg)
  {
    ...
    {
      NODELET_WARN("%s: Discarding gyro measurement above threshold %g > %g",
                   getName().c_str(),
                   w_temp,
  ...
  */
  void speed_cb(const marti_sensor_msgs::VelocityConstPtr& msg)
  {
    ...
    if (std::fabs((last_published_twist_->header.stamp - msg->header.stamp).toSec()) < 0.1)  // NOLINT
    {
    ...
    tf::Vector3 v(v_wheel_speed,
        0.0,
        0.0);
    tf::Vector3 v_var(v_wheel_speed_var,
  ...
  bool CheckRequiredSensorTimeouts()
  {
    if (require_wheel_encoders_ &&
        ((ros::Time::now() - last_odom_time_).toSec() > ODOM_TIMEOUT) )
    {
      NODELET_WARN_THROTTLE(1.0, "%s: Wheel speed odometry has timed out.",
                            getName().c_str());
      return false;
    }
    return true;
  ...
  void TimerCallback(const ros::TimerEvent& e)
  {
    ...
    geometry_msgs::TwistWithCovarianceStampedPtr temp_out(new geometry_msgs::TwistWithCovarianceStamped());
    ...
    {
      NODELET_ERROR_THROTTLE(1.0, "%s: Blocking output velocity because a "
                             "required sensor input is missing",
                             getName().c_str());
    }
  }

  void check_and_fix_twist(geometry_msgs::TwistWithCovarianceStamped& twist_in)
  {
    ...
    {
      last_longitudinal_vel_update_ = ros::Time::now();
    }
    ...
    {
      last_yaw_rate_update_ = ros::Time::now();
    }
    ...
    if (roll_rate_cov > 1e6 || pitch_rate_cov > 1e6)
    {
      ...
      NODELET_ERROR_THROTTLE(10.0, "%s: Pitch and roll rates not valid",
                             getName().c_str());
    ...
    double delay = (twist_in.header.stamp -
                    last_microstrain_vel_.header.stamp).toSec();
    if (delay > timeout_warning_)
    {
      NODELET_ERROR_THROTTLE(10.0, "%s: IMU may be dead.  Last update %f seconds ago.",
                             getName().c_str(),
                             delay);
    }
  ...
  void subscribe_to_topics()
  {
    ...
    Microstrain_Subscriber_ = swri::Subscriber(nh,
        "imu/data",
        100,
        &TwistAggregator::Microstrain_cb,
        this);

    Novatel_Subscriber_ = swri::Subscriber(nh,
        "gps",
        100,
        &TwistAggregator::Novatel_cb,
        this);

    Gyro_Subscriber_ = swri::Subscriber(nh,
        "yaw_rate",
        100,
        &TwistAggregator::gyro_cb,
        this);

    Wheel_Speed_Subscriber_ = swri::Subscriber(nh,
        "speed",
        100,
  ...
  void init_node()
  {
    ...
    last_twist_.header.stamp = ros::TIME_MIN;

    // Average 15 seconds of data from the gyro to compute the bias
    gyro_bias_.initialize(1000*gyro_bias_estimation_time_);
  ...
  {
    double elapsed_time = (ros::Time::now() -
        last_longitudinal_vel_update_).toSec();
  ...
  {
    double elapsed_time = (last_yaw_rate_update_ -
        ros::Time::now()).toSec();
  ...
  void init_diagnostics()
  {
    ...
    ros::NodeHandle& pnh = getPrivateNodeHandle();
    diagnostic_timer_ =
        pnh.createTimer(ros::Duration(1),
            &TwistAggregator::RunDiagnostics,
            this);
  }
};
}  // namespace sumet_state_estimator
...
PLUGINLIB_DECLARE_CLASS(sumet_state_estimator,
                        sumet_velocity3D_aggregator_nodelet,
                        sumet_state_estimator::TwistAggregator,
                        nodelet::Nodelet)
